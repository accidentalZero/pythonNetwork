...
# Простой TCP-клиент и сервер, которые отправляют и  получают 16 байт
...
def client(host, port, bytecount):
    ...
    print('Receiving all data the server sends back')
    received = 0
    while True:
        data = sock.recv(42)
        if not received:
...
if __name__ == '__main__':
    ...
    parser.add_argument('host', help='interdace the server listens at host the client sends to')
